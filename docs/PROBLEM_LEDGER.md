...
**Last updated:** 2026-08-29 (OPE-651 Formalist: `brooks-coloring` Level A landed zero-sorry. Namesake `brooks_colorable` residual. Not greedy. No novelty.)
...
|------------|--------|---------|-------------|---------|-------------------|
| `brooks-coloring` | graph theory | OPE-640 Scout #2; **OPE-650** Director approve unused slot; **OPE-651** Formalist Level A | **`informal`** (formalize-only, honest partial). Lean **zero-sorry** `ProofLab/Brooks.lean` Level A: `IsOddCycle` pin (connected + odd `card V` + 2-regular; **no** `cycleGraph`); complete exception `chromaticNumber_top_eq_maxDegree_add_one` (reuse `chromaticNumber_top`); odd-cycle exception `chromaticNumber_eq_three_of_isOddCycle` (`χ = 3 = Δ+1`) via Hierholzer circuit (reuse `eulerian_hierholzer_circuit`, not re-proved) + `Walk.three_le_chromaticNumber_of_odd_loop` + greedy 3-colouring; `theorem greedy_colorable` **reused** from `ProofLab.GreedyChromatic` (not labelled Brooks). `#print axioms` on the exception lemmas = `propext` / `Classical.choice` / `Quot.sound` (no `sorryAx`). `lake env lean ProofLab/Brooks.lean` EXIT=0; `lake build ProofLab` green. **Level B namesake residual:** `brooks_colorable` (connected, `G ≠ ⊤`, `¬ IsOddCycle` ⇒ `Colorable maxDegree`) not proved — named human-scale proof is Brooks 1941 / Diestel (delete deg-Δ vertex; extend; Kempe if needed) / Lovász contraction; Kempe/critical-graph is the known sink; **not** sorry-ed. **No novelty claim.** Not greedy. Not Vizing. Not 4CT/5CT. Do **not** re-prime greedy / Dilworth / Eulerian / König / Dirac. Do **not** expand into list-colouring Brooks. | Known classical (Brooks 1941 / Diestel); Mathlib v4.10.0 has `Colorable`/`maxDegree`/`chromaticNumber_top`/`three_le_chromaticNumber_of_odd_loop` defs only; ZERO `brooks` | `catalog/problems/brooks-coloring/STATEMENT.md`, Lean `ProofLab/Brooks.lean` |
...
2. **`brooks-coloring` (78). OPE-651 Formalist Level A landed** (zero-sorry exceptions + greedy reuse; namesake residual). Independent re-score, **not** an OPE-613 leftover rubber-stamp.
