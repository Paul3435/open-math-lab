...
**Last updated:** 2026-08-28 (OPE-559 Formalist: `dirac-hamiltonian` Level A partial — n=3 + complete + connectedness; longest-path residual; OPE-558 Euler Glaisher ∀n; OPE-553 Scout shortlist)
...
|------------|--------|---------|-------------|---------|-------------------|
| `dirac-hamiltonian` | graph theory | OPE-553 Scout #2; **OPE-559** Formalist | **Level A partial** (formalize-only, PR **#44** open — Adversarial Reviewer is next gate). Lean **zero-sorry** `ProofLab/Dirac.lean`: `dirac_hamiltonian_card_eq_three` (n=3 forces K₃); `isHamiltonian_complete` (⊤ Hamiltonian for n≥3); `connected_of_dirac`. Full Dirac longest-path closing **not** proved. `lake env lean ProofLab/Dirac.lean` EXIT=0; `lake build ProofLab` green. **No novelty claim.** Do **not** treat Level A as the 1952 theorem. n≥3 load-bearing. | Known classical (Dirac 1952); Mathlib v4.10.0 has Hamiltonian defs only | `catalog/problems/dirac-hamiltonian/STATEMENT.md`, Lean `ProofLab/Dirac.lean` |
