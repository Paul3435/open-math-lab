...
**Last updated:** 2026-08-29 (OPE-619 Formalist: Dilworth Level A honest partial — easy inequality + empty/chain/antichain/2-element specials; Fulkerson split graph + `Colorable 2` + uncovered-antichain lemmas. Level B ∀ not closed.)
...
|------------|--------|---------|-------------|---------|-------------------|
| `dilworth-poset` | order theory / combinatorics | OPE-613 Scout prime; **OPE-618** Director assign; **OPE-619** Formalist Level A | **`in_review`** (formalize-only, **honest partial**). Lean **zero-sorry** `ProofLab/Dilworth.lean`: STATEMENT wrappers `IsChainSet` / `IsAntichainSet` / `IsChainPartition` / `width`; **Level A** `width_le_chainPartition` + specials `dilworth_empty` / `dilworth_of_isChain_univ` / `dilworth_of_isAntichain_univ` / `dilworth_card_eq_two`. Fulkerson `splitGraph` on `α ⊕ α` (strict `<`), `splitGraph_colorable`, reuse `ProofLab.Konig`. **Level B ∀ finite poset not closed** (matching→chain-partition overran). Encoding Mathlib `PartialOrder` + `IsChain` + `IsAntichain`. `lake build ProofLab.Dilworth` + `lake build ProofLab` green. **No novelty claim.** Not König A/B. Not Hall. Not Mirsky. Not comparability graph. Finite only. | Known classical (Dilworth 1950 / Fulkerson 1956); Mathlib v4.10.0 has `IsChain`/`IsAntichain` defs only | `catalog/problems/dilworth-poset/STATEMENT.md`, Lean `ProofLab/Dilworth.lean` |
| `eulerian-hierholzer` | graph theory | OPE-574 Scout prime; **OPE-579** Formalist Level A; **OPE-591** Scout Level B re-score (#2); **OPE-597** Formalist Level B (PR **#51 MERGED**, `396e2a6`) | **`in_review`** (formalize-only, honest partial). Lean **zero-sorry** `ProofLab/Eulerian.lean`: Level A reused (`eulerian_k1` / `eulerian_cycle` / `eulerian_k2`). **Level B circuit CONSUMED:** `eulerian_hierholzer_circuit` (connected `G`, `card oddDeg = 0`, nonempty `edgeSet` → Eulerian circuit via longest trail; `IsCircuit` excludes `K_1` nil walk) + `eulerian_complete_odd` (`K_n`, odd `n ≥ 3`). Encoding Mathlib `Walk.IsEulerian` / `IsCircuit`. `lake build ProofLab.Eulerian` + `lake build ProofLab` green. **No novelty claim.** Do **not** import `Archive.*`. Not Königsberg. Not Dirac. **Do not re-prime Level A or Level B circuit/complete-odd.** STATEMENT trail clause (`card oddDeg = 2`) **not** closed — OPE-613 independently re-scored as shortlist #2 (start-at-odd longest trail; **no dummy-edge**). | Known classical (Euler 1736 / Hierholzer 1873); Mathlib v4.10.0 has `IsEulerian` + necessary `card_odd_degree` only; Trails.lean L26–29 existence TODO still open on the 2-odd trail | `catalog/problems/eulerian-hierholzer/STATEMENT.md`, Lean `ProofLab/Eulerian.lean` |
