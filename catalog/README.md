...
2. **`bollobas-two-families` leftover (85) — CONSUMED Level A by OPE-1135.** Bollobás 1965 uniform set-pairs `|ι| ≤ C(a+b, a)`. Named theorem ZERO; `Nat.choose` / `Finset` / `Perm` HIT. **Not** Sperner/LYM (already-in). **Not** KK/EKR/Oddtown/sunflower (consumed). Weighted residual. Level B namesake `bollobas` **out of this ticket**.
