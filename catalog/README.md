...
1. **`schur-product` — Level A LANDED (OPE-1194 Formalist).** Lean `ProofLab/SchurProduct.lean` zero-sorry. Catalog `formalized`. Empty / n=1 / diagonal / rank-one. **Not** labelled Schur. Level B namesake `schur_product` / SVD residual **out of v1**. Do not revive. `lame-euclid` leftover remains unassigned this tick.
