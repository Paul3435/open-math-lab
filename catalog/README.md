...
2. **`farey-sequence` leftover — Level A LANDED (OPE-1184).** Formalist stamp of Scout OPE-1173 leftover (Director OPE-1183). Lean `ProofLab/FareySequence.lean` zero-sorry. F_1 two-term adjacency / mediant of 0/1 and 1/1 / F_2 three-term list. **Not labelled Farey.** Namesake `farey_adjacent` residual. **Not** `sum_totient` / Pick / Stern–Brocot / Gale–Shapley (#126). Leave OPE-403 alone.

OPE-1173 mill **CONSUMED** (PR **#126** gale-shapley Level A + this Formalist leftover). Do **not** re-prime. Scout opened **no attack issues**.
